...
1. **Shred the solicitation** into individual requirements, see [what RFP shredding is](/blog/what-is-rfp-shredding/).
2. **Tag each requirement** with its source section (L, M, or C).
3. **Assign an owner and a response location** for each row.
4. **Track status** and keep the matrix in sync as the draft changes.

Doing this by hand on a 100-page RFP is slow and error-prone. The free [compliance matrix generator](/solutions/compliance-matrix-generator/) extracts requirements from pasted RFP text in your browser and exports them to CSV, a working matrix in seconds. GovHub then keeps it aligned to your draft as sections are written.
...
A compliance matrix is where a compliant proposal begins. For the full workflow around it, read [how to write a government proposal](/blog/how-to-write-a-government-proposal/), and understand the two sections that feed it in [what are Section L and Section M](/blog/what-are-section-l-and-m/).
